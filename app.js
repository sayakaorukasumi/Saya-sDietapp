...
// ---------- 運動データの後方互換 ----------
// 旧形式 {name, minutes} → 新形式 {name, value, unit}
...
function renderChart(range = "7") {
  ...
  chartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: !!profile.targetWeight },
      },
      scales: {
  ...
  list.innerHTML = entries.slice(0,50).map(([date,r]) => {
    ...
    if (r.exercises?.length > 0) {
      const exList = r.exercises.map(e => {
        ...
        const unitLabel = norm.unit === "rep" ? "回" : "分";
        if (norm.kcal) kcalTotal += norm.kcal;
        return `${norm.name}(${norm.value}${unitLabel})`;
      });
...
function renderProfileSummary() {
  ...
  if (latestWeight) {
    boxes += summaryBox("最新の体重", latestWeight, "kg", "");
  }
  if (p.targetWeight) {
    boxes += summaryBox("目標体重", p.targetWeight, "kg", "");
  }
  if (diff !== null) {
    const sign = diff > 0 ? "+" : "";
    const cls  = diff <= 0 ? "good" : "caution";
    boxes += summaryBox("目標まで", sign + diff.toFixed(1), "kg", cls);
  }
  if (p.height) {
    boxes += summaryBox("身長", p.height, "cm", "");
  }
  if (bmi) {
    const bmiCls = bmi < 18.5 ? "caution" : bmi < 25 ? "good" : "caution";
    boxes += summaryBox("BMI", bmi, "", bmiCls);
  }
